# 🚀 Руководство по развертыванию с PM2 на сервере

## 📋 Информация о сервере

- **IP:** 45.141.78.168
- **Менеджер процессов:** PM2
- **Без Docker**

---

## ✅ Предварительные требования на сервере

Убедитесь, что на сервере установлено:

- ✅ Node.js (v18 или выше)
- ✅ npm или pnpm
- ✅ PM2
- ✅ Git (опционально)

---

## 📝 Шаг 1: Подготовка локального проекта

### 1.1 Создайте файл .env для backend

В директории `backend/` создайте файл `.env`:

```bash
# Порт сервера
PORT=3001

# URL фронтенда для CORS
FRONTEND_URL=http://45.141.78.168

# Секретный ключ для JWT токенов (ОБЯЗАТЕЛЬНО ЗАМЕНИТЕ!)
JWT_SECRET=ваш_супер_секретный_ключ_минимум_32_символа

# Режим работы
NODE_ENV=production
```

**Сгенерировать JWT_SECRET:**

```powershell
# Windows PowerShell
$bytes = New-Object byte[] 32
(New-Object Security.Cryptography.RNGCryptoServiceProvider).GetBytes($bytes)
[Convert]::ToBase64String($bytes)
```

### 1.2 Создайте .env.local для фронтенда (если нет)

В корне проекта создайте `.env.local`:

```bash
NEXT_PUBLIC_API_URL=http://45.141.78.168:3001/api
```

---

## 📤 Шаг 2: Загрузка проекта на сервер

### Вариант A: Через Git (рекомендуется)

```bash
# На сервере
ssh root@45.141.78.168

# Перейдите в директорию
cd /opt

# Клонируйте репозиторий (если еще не сделано)
git clone <ваш-репозиторий> asian-goods-store
cd asian-goods-store

# Или обновите существующий
cd /opt/asian-goods-store
git pull origin main
```

### Вариант B: Через SCP/FTP

**С вашего компьютера (Windows PowerShell):**

```powershell
# Создайте архив (если есть tar)
tar -czf deploy.tar.gz `
    --exclude=node_modules `
    --exclude=.next `
    --exclude=backend/node_modules `
    --exclude=backend/uploads `
    --exclude=backend/database/store.db `
    --exclude=.git `
    *

# Загрузите на сервер
scp deploy.tar.gz root@45.141.78.168:/opt/asian-goods-store/

# На сервере распакуйте
ssh root@45.141.78.168
cd /opt/asian-goods-store
tar -xzf deploy.tar.gz
rm deploy.tar.gz
```

**Или используйте готовый скрипт:**

```powershell
.\deploy-to-server-pm2.ps1
```

---

## 🔧 Шаг 3: Установка зависимостей на сервере

```bash
# Подключитесь к серверу
ssh root@45.141.78.168

# Перейдите в директорию проекта
cd /opt/asian-goods-store

# Установите зависимости фронтенда
npm install
# или
pnpm install

# Установите зависимости бэкенда
cd backend
npm install
cd ..

# Соберите фронтенд для production
npm run build
```

---

## 🚀 Шаг 4: Запуск с PM2

### 4.1 Создайте директорию для логов

```bash
mkdir -p logs
```

### 4.2 Запустите приложение

```bash
# Запустить используя ecosystem.config.js
pm2 start ecosystem.config.js

# Или запустить вручную
pm2 start backend/server.js --name "asian-goods-backend" --time
pm2 start npm --name "asian-goods-frontend" -- start
```

### 4.3 Сохраните конфигурацию PM2

```bash
# Сохраните список процессов для автозапуска
pm2 save

# Настройте автозапуск при перезагрузке сервера
pm2 startup
# Выполните команду, которую PM2 предложит
```

---

## 📊 Шаг 5: Проверка работы

### 5.1 Проверьте статус процессов

```bash
pm2 status
# или
pm2 ls
```

Вы должны увидеть оба процесса в статусе "online":
```
┌─────┬───────────────────────────┬─────────┬─────────┬──────┐
│ id  │ name                      │ status  │ cpu     │ mem  │
├─────┼───────────────────────────┼─────────┼─────────┼──────┤
│ 0   │ asian-goods-backend       │ online  │ 0%      │ 50MB │
│ 1   │ asian-goods-frontend      │ online  │ 0%      │ 80MB │
└─────┴───────────────────────────┴─────────┴─────────┴──────┘
```

### 5.2 Проверьте логи

```bash
# Все логи
pm2 logs

# Только backend
pm2 logs asian-goods-backend

# Только frontend
pm2 logs asian-goods-frontend

# Последние 100 строк
pm2 logs --lines 100
```

### 5.3 Проверьте API

```bash
# Health check
curl http://localhost:3001/api/health

# Или с вашего компьютера
curl http://45.141.78.168:3001/api/health
```

### 5.4 Откройте в браузере

- **Сайт:** http://45.141.78.168:3000
- **Админ-панель:** http://45.141.78.168:3000/admin
- **API:** http://45.141.78.168:3001/api/health

---

## 🔧 Настройка Nginx (опционально, но рекомендуется)

Чтобы сайт работал на порту 80 (без :3000):

### 6.1 Установите Nginx (если не установлен)

```bash
sudo apt update
sudo apt install nginx -y
```

### 6.2 Создайте конфигурацию

```bash
sudo nano /etc/nginx/sites-available/asian-goods
```

Вставьте:

```nginx
server {
    listen 80;
    server_name 45.141.78.168;

    client_max_body_size 20M;

    # API прокси
    location /api/ {
        proxy_pass http://localhost:3001/api/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }

    # Статические файлы backend (uploads)
    location /uploads/ {
        alias /opt/asian-goods-store/backend/uploads/;
    }

    # Frontend
    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_cache_bypass $http_upgrade;
    }

    # Next.js статика
    location /_next/ {
        proxy_pass http://localhost:3000/_next/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
    }
}
```

### 6.3 Активируйте конфигурацию

```bash
# Создайте символическую ссылку
sudo ln -s /etc/nginx/sites-available/asian-goods /etc/nginx/sites-enabled/

# Удалите дефолтную конфигурацию
sudo rm /etc/nginx/sites-enabled/default

# Проверьте конфигурацию
sudo nginx -t

# Перезапустите Nginx
sudo systemctl restart nginx

# Включите автозапуск
sudo systemctl enable nginx
```

После этого сайт будет доступен по адресу: **http://45.141.78.168** (без порта)

---

## 🤖 Шаг 7: Настройка Telegram бота

### 7.1 Получите токен бота

1. Откройте Telegram → [@BotFather](https://t.me/BotFather)
2. Отправьте: `/newbot`
3. Следуйте инструкциям
4. Скопируйте токен

### 7.2 Получите Chat ID

1. Откройте Telegram → [@userinfobot](https://t.me/userinfobot)
2. Отправьте любое сообщение
3. Скопируйте ваш Chat ID

### 7.3 Настройте в админ-панели

1. Откройте: http://45.141.78.168/admin (или http://45.141.78.168:3000/admin)
2. Войдите под администратором
3. Перейдите в **"Настройки Telegram"**
4. Введите:
   - **Bot Token**
   - **Chat ID**
   - **Включить**: ✅
5. Нажмите **"Тест подключения"**
6. Если тест прошел успешно → **"Сохранить"**

---

## 🔄 Обновление проекта

### Способ 1: Через Git

```bash
ssh root@45.141.78.168
cd /opt/asian-goods-store

# Загрузите изменения
git pull origin main

# Установите зависимости (если изменились)
npm install
cd backend && npm install && cd ..

# Пересоберите фронтенд
npm run build

# Перезапустите приложение
pm2 restart all

# Проверьте логи
pm2 logs --lines 50
```

### Способ 2: Автоматический скрипт

**С вашего компьютера:**

```powershell
.\deploy-to-server-pm2.ps1
```

---

## 📋 Полезные PM2 команды

### Управление процессами

```bash
# Статус всех процессов
pm2 status
pm2 ls

# Детальная информация
pm2 show asian-goods-backend

# Перезапуск
pm2 restart all
pm2 restart asian-goods-backend
pm2 restart asian-goods-frontend

# Остановка
pm2 stop all
pm2 stop asian-goods-backend

# Запуск
pm2 start all
pm2 start asian-goods-backend

# Удаление из списка
pm2 delete asian-goods-backend
pm2 delete all

# Перезагрузка с 0 downtime
pm2 reload all
```

### Логи

```bash
# Просмотр логов в реальном времени
pm2 logs

# Логи конкретного процесса
pm2 logs asian-goods-backend

# Последние N строк
pm2 logs --lines 100

# Очистить логи
pm2 flush

# Файлы логов находятся в
ls -la logs/
```

### Мониторинг

```bash
# Мониторинг в реальном времени
pm2 monit

# Веб-интерфейс (опционально)
pm2 plus
```

### Автозапуск

```bash
# Сохранить текущий список процессов
pm2 save

# Настроить автозапуск при старте системы
pm2 startup

# Отключить автозапуск
pm2 unstartup
```

---

## 🐛 Решение проблем

### Backend не запускается

```bash
# Проверьте логи
pm2 logs asian-goods-backend --lines 100

# Проверьте .env файл
cat backend/.env

# Проверьте порт
netstat -tlnp | grep 3001
# или
ss -tlnp | grep 3001

# Если порт занят - найдите процесс
lsof -i :3001
# Убейте процесс
kill -9 <PID>

# Перезапустите
pm2 restart asian-goods-backend
```

### Frontend не запускается

```bash
# Проверьте сборку
npm run build

# Проверьте логи
pm2 logs asian-goods-frontend --lines 100

# Проверьте .env.local
cat .env.local

# Пересоберите и перезапустите
npm run build
pm2 restart asian-goods-frontend
```

### Telegram уведомления не работают

```bash
# Войдите в базу данных
cd /opt/asian-goods-store/backend
sqlite3 database/store.db

# В SQLite:
SELECT * FROM telegram_settings;
.exit

# Проверьте логи
pm2 logs asian-goods-backend | grep -i telegram

# Перезапустите backend
pm2 restart asian-goods-backend
```

### Проблемы с памятью

```bash
# Проверьте использование памяти
pm2 status

# Увеличьте лимит в ecosystem.config.js
# max_memory_restart: '1G'

# Перезапустите
pm2 restart all
```

---

## 💾 Резервное копирование

### Автоматический backup базы данных

```bash
# Создайте скрипт
sudo nano /root/backup-db.sh
```

Вставьте:

```bash
#!/bin/bash
BACKUP_DIR="/backup/asian-goods"
DATE=$(date +%Y%m%d_%H%M%S)
mkdir -p $BACKUP_DIR

cd /opt/asian-goods-store/backend/database
cp store.db "$BACKUP_DIR/backup_${DATE}.db"

# Удалить старые backup'ы (старше 7 дней)
find $BACKUP_DIR -name "backup_*.db" -mtime +7 -delete

echo "Backup created: $BACKUP_DIR/backup_${DATE}.db"
```

Сделайте исполняемым:

```bash
chmod +x /root/backup-db.sh
```

Добавьте в cron:

```bash
crontab -e
```

Добавьте строку (backup каждый день в 2:00):

```
0 2 * * * /root/backup-db.sh >> /var/log/asian-goods-backup.log 2>&1
```

---

## 🔒 Безопасность

### Настройте Firewall

```bash
# Разрешите нужные порты
sudo ufw allow 22/tcp    # SSH
sudo ufw allow 80/tcp    # HTTP
sudo ufw allow 443/tcp   # HTTPS (для будущего SSL)

# Если нет Nginx, разрешите прямой доступ к приложению
sudo ufw allow 3000/tcp
sudo ufw allow 3001/tcp

# Включите firewall
sudo ufw enable

# Проверьте статус
sudo ufw status
```

### Защитите .env файлы

```bash
chmod 600 backend/.env
chmod 600 .env.local
```

---

## 📊 Мониторинг

```bash
# Использование ресурсов
pm2 monit

# Или в консоли
watch -n 2 'pm2 status && echo "" && free -h && df -h'

# Логи в реальном времени
pm2 logs --lines 50
```

---

## ✅ Чек-лист развертывания

- [ ] Код загружен на сервер
- [ ] Файл `backend/.env` создан и настроен
- [ ] JWT_SECRET сгенерирован и установлен
- [ ] Зависимости установлены (npm install)
- [ ] Backend зависимости установлены
- [ ] Frontend собран (npm run build)
- [ ] PM2 процессы запущены
- [ ] PM2 сохранен (pm2 save)
- [ ] PM2 автозапуск настроен (pm2 startup)
- [ ] Nginx настроен (опционально)
- [ ] Firewall настроен
- [ ] Telegram бот настроен через админ-панель
- [ ] Тестовый заказ создан и уведомление получено
- [ ] Автоматические backup'ы настроены

---

## 🎉 Готово!

Ваш интернет-магазин развернут на сервере с PM2!

**Проверьте:**
- 🌐 Сайт: http://45.141.78.168 (или :3000)
- 🔧 Админ: http://45.141.78.168/admin
- 📋 API: http://45.141.78.168:3001/api/health
- 📱 Telegram уведомления работают

**Команды для быстрого доступа:**

```bash
# SSH
ssh root@45.141.78.168

# Перейти в проект
cd /opt/asian-goods-store

# Статус
pm2 status

# Логи
pm2 logs

# Перезапуск
pm2 restart all
```

